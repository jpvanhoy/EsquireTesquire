...
class Services_ProofOfOrdering extends Services_Base
{
	public function getProofOfOrderingList($token, $maxResults = 1000) {
		if($this->security->hasActiveSession($token, $this->serviceName)) {
			...
			$query->select('*');
			$query->from('Models_Solaria_ProofOfOrdering');
