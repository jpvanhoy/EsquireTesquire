...
class Services_AppearanceType extends Services_Base
{
	public function getAppearanceTypeList($token, $maxResults = 1000) {
		if($this->security->hasActiveSession($token, $this->serviceName)) {
			...
			$query->from('Models_Solaria_AppearanceType');
