<?php
/**
 * A service to provide a ReporterTimeAndDistance list and so forth and whatnot.
 * @author Michael Locke @ Nov 12, 2012 10:35:45 AM
 */
class Services_ReporterTimeAndDistance extends Services_Base
{
	public function getReporterTimeAndDistanceList($token, $maxResults = 1000000) {
		if($this->security->hasActiveSession($token, $this->serviceName)) {
			...
			$query->select('rtd.*');
			$query->from('Models_Solaria_ReporterTimeAndDistance rtd');
