 ...
 */
class Rest_DeliveryMethodController extends Rest_AbstractController {
	public function indexAction() {
		...
		try {
			...
			if($valid) {
				$result	= $service->getListOfBaseDeliveryMethods($this->token);
	...
	public function getAction() {
		$orderDeponentDeliverableId = $this->_getParam('orderDeponentDeliverableId');
		$this->setService(Esquire_Service_Factory::getService('Deliverable'));

		if(empty($orderDeponentDeliverableId)) {
		...
		try {
			...
			if($valid) {
                $result = $this->getService()->getDeliveryMethodsForOrderDeponentDeliverable($this->token, $orderDeponentDeliverableId);
				$this->processResult($result);
			} else {
				$this->setResponseCode(Rest_RestHelper::HTTP_FORBIDDEN);
				$this->setResponseData('Invalid or expired token provided.', TRUE);
